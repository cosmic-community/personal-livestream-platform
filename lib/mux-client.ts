...
interface MuxLiveStream {
  id: string
  status: string
  stream_key?: string // Make optional since it can be undefined
...
class MuxClient {
  ...
  constructor(config?: Partial<MuxConfig>) {
    ...
    // Fixed: Use correct Mux SDK initialization without RequestOptions
  ...
  async createLiveStream(options: {
    ...
    newAssetSettings?: {
      playbackPolicy?: 'public' | 'signed'
      mp4Support?: 'none' | 'standard' // Fixed: Remove invalid 'capped-1080p' option
    ...
    try {
      ...
      // Fixed: Use correct Mux SDK method name
      const liveStream = await this.mux.Video.LiveStreams.create({
        ...
        new_asset_settings: {
          ...
          mp4_support: options.newAssetSettings?.mp4Support || 'standard', // Fixed: Use valid option
      ...
      // Fixed: Add null check for stream_key
      ...
      const streamData = {
        id: liveStream.id,
        status: liveStream.status,
        streamKey: liveStream.stream_key, // Now safe to use after null check
  ...
  async getLiveStream(liveStreamId: string) {
    try {
      ...
      return {
        id: liveStream.id,
        status: liveStream.status,
        streamKey: liveStream.stream_key || '', // Fixed: Handle undefined stream_key
        playbackIds: liveStream.playback_ids || [],
        rtmpUrl: `rtmps://global-live.mux.com:443/live/${liveStream.stream_key || ''}`,
        reconnectWindow: liveStream.reconnect_window,
        reducedLatency: liveStream.reduced_latency,
  ...
  async enableLiveStream(liveStreamId: string) {
    try {
      // Fixed: Use correct method name and handle void return type properly
